...
###############################################################################################################################
 ...
 ###############################################################################################################################
       # generator variables
        ...
        self.gen_loss = tf.reduce_sum(tf.subtract(tf.reduce_mean(self.obs, axis=0), tf.reduce_mean(self.gen_res, axis=0)))
...
###############################################################################################################################
    ...
    def langevin_dynamics_generator(self, z_arg):
        ...
        def body(i, z):
            ...
            grad = tf.gradients(gen_res, z, name='gen_des')[0]
            z = z - 0.5 * self.delta2 * self.delta2 * (z / self.sigma2 / self.sigma2 - grad) + self.delta2 * noise
            return tf.add(i, 1), z
